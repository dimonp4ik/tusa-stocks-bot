...
from config import (
    ...
    POST_TP1_WEAK_CLOSE_PROGRESS, STOCK_VENUE_FILTER_MODE, STOCK_VENUE_FILTER_PROFILE,
    DEPLOYMENT_MODE,
)
from src.binance_client import (
    get_klines, get_current_price, get_xperp_instruments, get_xperp_price, get_klines_xperp,
)
from src.stock_venue_router import AUDITED_SYMBOLS, stock_venue_setups
...
from src.db import (
    ...
    get_symbols_performance, upsert_user, get_user_by_id, get_all_users, get_users_count,
    add_dynamic_admin, remove_dynamic_admin, get_dynamic_admins, is_dynamic_admin,
    get_dynamic_role, delete_signal, get_signals_count, get_signals_page,
    get_distinct_signal_symbols, get_bot_state, set_bot_state,
    log_setup_candidate_once, mark_setup_sent, get_setups_by_date, mark_setup_blocked,
...
def _build_and_send_report(chat_id: int, message_id, since_ts: float,
    ...
    try:
        ...
        A(f"  СТРАТЕГИЯ: {STOCK_VENUE_FILTER_PROFILE} ({STOCK_VENUE_FILTER_MODE})")
        A("  КЛОД: отключён от решений (2026-09-16)")
...
@app.route("/")
def health():
    return f"TUSA Stocks Bot is running ({DEPLOYMENT_MODE}).", 200
...
# Session windows. These three were lost when the venue strategy was written in
# (backtest.py still documents _in_extended_window as its original); the new scan
# calls _in_extended_window on every closed-market tick, so without them run_scan
# raised NameError every 5 minutes outside regular hours. Restored 2026-09-16.
...
def _kb_settings():
    # Only controls that actually reach the venue strategy belong here. The
    # session toggle gates its scan and the block list filters its symbols;
    # both were unreachable from the menu until 2026-09-16 while still being
    # read by the code.
    return {"inline_keyboard": [
        [{"text": "🧩 Активные фильтры", "callback_data": "adm_shadow_status"}],
        [{"text": "🌙 Сигналы вне сессии", "callback_data": "adm_night_toggle"}],
        [{"text": "🚫 Блокировка тикеров", "callback_data": "adm_manblock"}],
...
def _handle_admin_callback(callback_id: str, chat_id: int,
    ...
    if data == "adm_sec_trading":
        ...
        # The button existed with no handler at all, so pressing it did nothing.
        # It now reports what the paper strategy actually depends on: the venue
        # instrument list, the candle feed, the database, and the last scan.
        import time as _t
        lines_h = ["🩺 *Проверка сервисов*", ""]
        try:
            inst = get_xperp_instruments()
            covered = sorted(x for x in AUDITED_SYMBOLS if x[:-4] in set(inst))
            lines_h.append(f"✅ X-Perp: инструментов {len(inst)}, "
                           f"из нашего списка доступно {len(covered)}/{len(AUDITED_SYMBOLS)}")
            missing = [x for x in AUDITED_SYMBOLS if x[:-4] not in set(inst)]
            if missing:
                lines_h.append(f"   ⚠️ нет инструмента: {', '.join(x[:-4] for x in missing[:8])}")
        except Exception as exc:
            lines_h.append(f"❌ X-Perp недоступен: {exc}")
        try:
            qqq = get_klines_xperp("QQQUSDT", limit=5)
            lines_h.append("✅ Свечи QQQ: получены" if qqq else "❌ Свечи QQQ: пусто")
        except Exception as exc:
            lines_h.append(f"❌ Свечи QQQ: {exc}")
        try:
            open_n = len(get_open_signals())
            lines_h.append(f"✅ База: открытых paper-сделок {open_n}")
        except Exception as exc:
            lines_h.append(f"❌ База: {exc}")
        st = _last_scan_stats
        if st["ts"] > 0:
            lines_h.append(f"✅ Последний скан: {int((_t.time() - st['ts']) / 60)} мин назад, "
                           f"тикеров {st['coins']}, сигналов {st['setups']}")
        else:
            lines_h.append("⏳ Скан ещё не отработал в этом процессе")
        lines_h.append("")
        lines_h.append(f"Профиль: `{STOCK_VENUE_FILTER_PROFILE}` ({STOCK_VENUE_FILTER_MODE})")
        from config import AUTOTRADE_ENABLED as _at_on
        lines_h.append(f"Реальные ордера: {'РАЗРЕШЕНЫ' if _at_on else '*ЗАПРЕЩЕНЫ КОДОМ*'}")
        _edit_admin_text(chat_id, message_id, chr(10).join(lines_h), _kb_settings())
...
def webhook():
    ...
    # 🤖 Автотрейдинг — только в личке, только для допущенных админом
        ...
        _reply(chat_id,
               "🛡 *ТЕНЕВОЙ РЕЖИМ*\n\n"
               f"Работает профиль `{STOCK_VENUE_FILTER_PROFILE}`.\n"
...
def _cooldowns_load() -> None:
    ...
    the signal cooldown to zero, and a symbol whose trade had already CLOSED
    could be re-signalled right after a restart.
    """
...
# ── Reject cooldown ────────────────────────────────────────────────────────────
# Kept only because the cache is loaded and saved with the signal cooldown. The
# rejection path itself belonged to the Claude gate and was removed on
# 2026-09-16, so nothing writes to this any more.
...
def _stock_venue_analysis(symbol: str, setup: dict) -> dict:
    direction = setup["direction"]
    return {
        "symbol": symbol, "direction": direction, "decision": direction,
        "confidence": "HIGH", "risk_score": 1,
        "reason": f"Frozen stock X-Perp module: {setup['module']}",
        "counter": "The recent direct-venue regime may change after entry",
        "current_price": setup["entry"], "market_price": setup["entry"],
        "atr": setup["atr"], "fixed_stop_atr": 1.0,
...
def _run_stock_venue_strategy_scan() -> None:
    ...
    # The admin toggle has to reach this scan too, or it is a control that
    # silently does nothing. Same order the old scan used: regular session,
    # then the all-or-nothing off-session switch, then the measured windows.
    if (not is_market_open() and not _off_session_enabled()
            and not _in_extended_window()):
        log.info("Stock venue scan skipped: US market is closed")
        return
    ...
    targets = sorted(s for s in AUDITED_SYMBOLS if s[:-4] in available)
    fetch_symbols = sorted(set(targets) | {"QQQUSDT", "SPYUSDT"})
    fetched = {}

    from concurrent.futures import ThreadPoolExecutor, as_completed
    def _fetch(symbol: str):
        return symbol, get_klines_xperp(symbol, limit=3000)
    with ThreadPoolExecutor(max_workers=2) as pool:
        for future in as_completed([pool.submit(_fetch, symbol) for symbol in fetch_symbols]):
            try:
                symbol, candles = future.result()
                if candles:
                    fetched[symbol] = candles
            except Exception as exc:
                log.warning("Stock venue candle fetch failed: %s", exc)

    qqq, spy = fetched.get("QQQUSDT"), fetched.get("SPYUSDT")
    ...
    for symbol in targets:
        ...
        try:
            ...
            rows = stock_venue_setups(
                candles, qqq, spy, market_price=float(market),
                profile=STOCK_VENUE_FILTER_PROFILE, symbol=symbol,
            )
            for row in rows:
                row["signal_bar_ts"] = candles["time"][-1]
                analysis = _stock_venue_analysis(symbol, row)
...
def run_scan():
    """Publish paper signals from the frozen X-Perp router.

    The old SMC scan was removed on 2026-09-16: it is not a fallback, it is a
...
def start_bot():
    ...
    try:
        ...
        if not skip:
            open(_flag, "w").close()
            send_status(
                "🧪 *Stocks Bot Online — SHADOW*\n"
                f"Профиль `{STOCK_VENUE_FILTER_PROFILE}`, скан каждые "
                f"{SCAN_INTERVAL_MINUTES} мин в сессию США.\n"
    ...
    scheduler.start()
    log.info("Scheduler: venue scan every 5 min (:01/:06/...), paper monitor every 1 min")
