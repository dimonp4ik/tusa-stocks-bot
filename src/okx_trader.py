...
def get_position_avg_px(creds: dict, inst_id: str) -> float | None:
    ...
    market order actually fills on the X-Perp seconds later, at a price that
    differs by the basis plus slippage. Stops derived from the signal price are
    therefore off by that gap, always in the same direction.
    """
