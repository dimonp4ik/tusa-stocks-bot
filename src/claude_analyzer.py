...
- RSI: 14-period on 15m. The upstream filter ALREADY drops LONGs above 72 and SHORTs below 28, so a genuinely overextended entry never reaches you — every RSI you see is inside those caps (verified: zero trades beyond them in the 1,676-trade seed). Inside the caps the level carries no measurable edge either way on this desk (LONG RSI>70 / SHORT RSI<30: +0.55R vs +0.60R for the rest, and the sign flips year to year — noise). So RSI is context, never a reason to reject on its own.
...
- Str: 15m swing structure at signal time. bull = higher-high + higher-low sequence. bear = lower-high + lower-low. range = neither. Use this to gauge whether entry is WITH or AGAINST the short-term structure. Counter-structure is NOT by itself a warning: measured over the 1,676-trade seed it is a wash (+0.54R counter vs +0.60R aligned, with the sign flipping year to year), so it describes the entry rather than condemning it. Mechanically a retest that cuts against the 15m swing is often exactly the deep pullback the FVG/OB zone exists to catch. (Separate from HTF trend: fighting a 1h/4h/1d trend IS a real concern, and the filter blocks the worst of it upstream.)
...
5. Do NOT reject on RSI level alone. The overextension caps (LONG>72, SHORT<28) are enforced upstream, so nothing you see has cleared them, and inside the caps the level shows no measurable edge either way here. What DOES make an entry a chase is DISTANCE FROM THE ZONE (see Z:), not the oscillator.
...
def _global_feedback() -> str:
    ...
    # minimum on the rejected side and none on the sent side, so the comparison
    # could rest on 2-3 approved trades: two lucky sends read as 100% and
    # silently switch the corrector off at the moment it is most needed.
    ...
    # the rejected pool, which needs no second sample.
...
def _scorecard_feedback() -> str:
    ...
    def _rband(lo, hi):
        ...
        for r in rows:
            try:
                ...
                continue
        return out
    # Four bands, not three. Measured on the crypto bot 2026-08-03: risk 4-5 hit
    # TP1 77%, risk 6-7 only 54%, risk 8+ 87% — a U, with the MIDDLE band the
    # miscalibrated one. The old 0-3/4-6/7-10 split straddled that break (6 fell
    # low, 7 fell high), smearing the U into a smooth "riskier does better" —
    # the opposite of the actual lesson. Bands stay evenly spaced, just finer.
    ...
    # Confidence buckets over ALL evaluated setups. Restricting to LONG/SHORT
    # made this half structurally unable to ever fire: Claude uses MEDIUM for
    # essentially every directional verdict and LOW for essentially every
    # rejection, so the directional-only view has ONE bucket and the ">= 2
    # buckets" guard silently dropped the whole confidence block. It cannot
    # tell him his confidence is noise while requiring him to already use the
    # scale it is measuring.
    cparts = []
